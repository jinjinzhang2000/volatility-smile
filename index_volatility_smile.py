...
def calculate_etf_volatility_smile(df, etf_type, trade_date_str, risk_free_rate=0.025):
    ...
    for maturity in sorted(df['maturity'].dropna().unique()):
        ...
        # Use settle price, fallback to close
        calls['price'] = calls['settle'].fillna(calls['close'])
        puts['price'] = puts['settle'].fillna(puts['close'])
        ...
        for K in strikes:
            ...
            for _, row in strike_opts.iterrows():
                price = row['settle'] if pd.notna(row['settle']) and row['settle'] > 0 else row['close']
            ...
            price = best_opt['settle'] if pd.notna(best_opt['settle']) and best_opt['settle'] > 0 else best_opt['close']
...
def process_index_option(code, data_dir='index_options_data', trade_date=None):
    """
...
def process_all_index_options(data_dir='index_options_data', trade_date=None):
